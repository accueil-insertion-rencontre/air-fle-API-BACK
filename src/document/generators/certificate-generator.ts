  ...
  private createCertificatePDF(data: CertificateData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.rect(80, startY, boxWidth, boxHeight)
           .fillColor('#f8f9fa')
           .fill()
        ...
        doc.fontSize(11)
           .font('Helvetica')
           .text(`a suivi avec succès une formation de français langue étrangère dans notre établissement.`, 
             70, doc.y, { 
               width: contentWidth, 
               align: 'left' 
             });
        ...
        doc.fontSize(10)
           .text(`Période de formation : du ${data.startDate} au ${data.endDate}`, 
             70, doc.y, { width: contentWidth, align: 'left' });

        doc.moveDown(0.8);

        doc.fontSize(11)
           .text('Cette formation lui permet de justifier de ses compétences en français auprès des autorités compétentes.', 
             70, doc.y, {
               width: contentWidth,
               align: 'left'
             });
        ...
        doc.moveDown(0.5);
        doc.text('En foi de quoi, nous lui délivrons le présent certificat pour servir et valoir ce que de droit.', 
          70, doc.y, {
            width: contentWidth,
            align: 'left'
          });
        ...
        doc.fontSize(9)
           .fillColor('#7f8c8d')
           .text('Ce certificat est valable 6 mois à compter de sa date d\'émission.', 
             70, 750, {
               width: contentWidth,
               align: 'center'
             });
