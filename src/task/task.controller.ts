...
export class TaskController {
  ...
  @Get()
  @ApiOperation({ summary: 'Récupérer toutes les tâches de l\'utilisateur avec statistiques' })
  @ApiResponse({ status: 200, description: 'Liste des tâches avec statistiques récupérée avec succès' })
  async findAllTasks(@Request() req): Promise<any> {
    return this.taskService.getAllTasksWithStats(req.user.id);
  }
  ...
  async findOneTask(@Request() req, @Param('id') id: string): Promise<Task | null> {
    ...
    if (task.user_id !== req.user.id) {
      throw new ForbiddenException('Vous ne pouvez accéder qu\'à vos propres tâches');
    }
  ...
  @ApiParam({ name: 'id', description: 'ID de la tâche' })
  async getTaskStatistics(@Request() req, @Param('id') id: string): Promise<any> {
    // Vérifier que la tâche appartient à l'utilisateur
    const task = await this.taskService.findOne(id);
    if (!task) {
      throw new NotFoundException('Tâche non trouvée');
    ...
    if (task.user_id !== req.user.id) {
      throw new ForbiddenException('Vous ne pouvez accéder qu\'aux statistiques de vos propres tâches');
    }
  ...
  async createSubtask(
    ...
    if (task.user_id !== req.user.id) {
      throw new ForbiddenException('Vous ne pouvez ajouter des sous-tâches qu\'à vos propres tâches');
    }
  ...
  @ApiResponse({ status: 200, description: 'Liste des sous-tâches récupérée avec succès' })
  @ApiParam({ name: 'taskId', description: 'ID de la tâche' })
  async findAllSubtasks(@Request() req, @Param('taskId') taskId: string): Promise<SubtaskWithRelations[]> {
    // Vérifier que la tâche appartient à l'utilisateur
    const task = await this.taskService.findOne(taskId);
    if (!task) {
      throw new NotFoundException('Tâche non trouvée');
    ...
    if (task.user_id !== req.user.id) {
      throw new ForbiddenException('Vous ne pouvez accéder qu\'aux sous-tâches de vos propres tâches');
    }
  ...
  async findOneSubtask(@Request() req, @Param('id') id: string): Promise<SubtaskWithRelations | null> {
    ...
    // Vérifier que la tâche parent appartient à l'utilisateur
    if (subtask.task.user_id !== req.user.id) {
      throw new ForbiddenException('Vous ne pouvez accéder qu\'à vos propres sous-tâches');
    }
  ...
  @ApiResponse({ status: 200, description: 'Sous-tâche mise à jour avec succès' })
  @ApiParam({ name: 'id', description: 'ID de la sous-tâche' })
  async updateSubtask(
    @Request() req,
    @Param('id') id: string,
    ...
    // Vérifier que la tâche parent appartient à l'utilisateur
    if (subtask.task.user_id !== req.user.id) {
      throw new ForbiddenException('Vous ne pouvez modifier que vos propres sous-tâches');
    }
  ...
  @ApiParam({ name: 'id', description: 'ID de la sous-tâche' })
  async deleteSubtask(@Request() req, @Param('id') id: string): Promise<SubtaskWithRelations> {
    const subtask = await this.subtaskService.findOne(id);
    if (!subtask) {
      throw new NotFoundException('Sous-tâche non trouvée');
    }
    ...
    // Vérifier que la tâche parent appartient à l'utilisateur
    if (subtask.task.user_id !== req.user.id) {
      throw new ForbiddenException('Vous ne pouvez supprimer que vos propres sous-tâches');
    }
  ...
  @ApiResponse({ status: 200, description: 'Statut de la sous-tâche basculé avec succès' })
  @ApiParam({ name: 'id', description: 'ID de la sous-tâche' })
  async toggleSubtaskStatus(@Request() req, @Param('id') id: string): Promise<SubtaskWithRelations> {
    const subtask = await this.subtaskService.findOne(id);
    if (!subtask) {
      throw new NotFoundException('Sous-tâche non trouvée');
    }
