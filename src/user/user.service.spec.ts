...
describe('UserService', () => {
  ...
  describe('update', () => {
    it('devrait mettre à jour un utilisateur', async () => {
      ...
      const result = await userService.update('test-id', {
        firstname: 'Jacques',
      });
    ...
    it("devrait lancer une NotFoundException si l'utilisateur n'existe pas", async () => {
      ...
      await expect(
        userService.update('invalid-id', { firstname: 'Jacques' }),
    ...
    it('devrait propager les erreurs inattendues de la base de données', async () => {
      ...
      await expect(
        userService.update('test-id', { firstname: 'Jacques' }),
  ...
  describe('remove', () => {
    ...
    it("devrait lancer une NotFoundException si l'utilisateur n'existe pas", async () => {
      ...
      await expect(userService.remove('invalid-id')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('devrait propager les erreurs inattendues de la base de données', async () => {
      ...
      await expect(userService.remove('test-id')).rejects.toThrow(Error);
    });
  });
});
