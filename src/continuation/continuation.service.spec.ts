...
describe('ContinuationService', () => {
  ...
  beforeEach(() => {
    ...
    continuationService = new ContinuationService(prismaService as any);
  });
  ...
  describe('findByStudent', () => {
    ...
    it('devrait retourner null si aucune continuation n\'est trouvée pour l\'étudiant', async () => {
      ...
      const result = await continuationService.findByStudent('non-existent-student-id');
  ...
  describe('create', () => {
    ...
    it('devrait gérer les erreurs lors de la création', async () => {
      ...
      await expect(continuationService.create(continuationData)).rejects.toThrow(Error);
    });
  });
  ...
  describe('update', () => {
    it('devrait mettre à jour une continuation', async () => {
      ...
      const result = await continuationService.update('continuation-id', updateData);
    ...
    it('devrait gérer les erreurs si la continuation n\'existe pas', async () => {
      ...
      await expect(continuationService.update('non-existent-id', updateData)).rejects.toThrow(Error);
    });
  });
  ...
  describe('delete', () => {
    ...
    it('devrait gérer les erreurs si la continuation n\'existe pas', async () => {
      ...
      await expect(continuationService.delete('non-existent-id')).rejects.toThrow(Error);
    });
  });
}); 