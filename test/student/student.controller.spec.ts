...
describe('StudentController - Tests Simples', () => {
  ...
  it('devrait simuler erreur 404 pour UUID invalide', () => {
    ...
    const response = {
      ...
      data: found || null,
      message: found ? 'Étudiant trouvé' : `Étudiant avec l'UUID ${requestedUuid} non trouvé`
    };
  ...
  it('devrait simuler création étudiant avec succès', () => {
    ...
    const isValid = studentData.student_firstname && 
  ...
  it('devrait simuler récupération étudiant par UUID', () => {
    const students = [
      { student_uuid: 'uuid-123', student_firstname: 'John', student_lastname: 'Doe' },
      { student_uuid: 'uuid-456', student_firstname: 'Jane', student_lastname: 'Smith' }
    ];
  ...
  it('devrait simuler erreur 400 pour données invalides', () => {
    ...
    const hasAllFields = requiredFields.every(field => 
      invalidData[field as keyof typeof invalidData]
    );

    // Simulation validation
    const response = {
      status: hasAllFields ? 201 : 400,
      message: hasAllFields ? 'Données valides' : 'Champs obligatoires manquants'
    };
